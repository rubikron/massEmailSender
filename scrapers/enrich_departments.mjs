...
import { DEFAULT_DEPARTMENT, SOURCE_DESCRIPTION } from './config.mjs';

// Load entries and rich data
...
// Session tags that match entries
const sessionTags = {
...
// Department inference from keywords in title/abstract
const departmentPatterns = [
  ...
  { name: 'Physics', keywords: ['physics', 'astrophysics', 'astronom', 'lensing', 'stellar', 'galactic', 'dark matter', 'exoplanet', 'technosignatur', 'cosmolog', 'gravitational', 'quantum physics', 'particle physics', 'galaxy'] },
...
function inferDepartment(entry) {
  // First check if tags match session tags
  const raw = rawEntries.find(r =>
    r.firstName === entry.name.split(' ')[0] &&
    r.lastName === entry.name.split(' ').slice(-1)[0]
  );
...
// Update departments
const enriched = richData.map(entry => {
  entry.department = inferDepartment(entry);
  return entry;
});

// Regenerate CSV with departments
...
console.log(`Updated CSV with departments: ${enriched.length} contacts`);
