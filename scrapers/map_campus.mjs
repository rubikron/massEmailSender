...
// Session tag mapping (from generate_csv.mjs)
const sessionTags = {
...
// Check if entry has a valid email (pass-through filter)
function hasValidEmail(entry) {
...
function inferTrack(entry) {
  ...
  const techWords = ['computer', 'ai', 'machine learning', 'neural', 'algorithm', 'software',
    ...
    'mathematic', 'statistic', 'optimization', 'control theory', 'conformal predict',
    'microservice', 'distributed', 'computer vision', 'generative', 'llm', 'sparse autoencoder',
    'multi-agent', 'topolog', 'number theor', 'probability', 'calculus',
  ...
  const healthWords = ['health', 'medicine', 'medical', 'biomedical', 'clinical', 'patient',
    ...
    'healthcare', 'hospital', 'pharmacolog', 'psycholog', 'cognition', 'mental health',
    'opioi', 'mesenchymal', 'stem cell', 'axon', 'loc o motion', 'drosophila',
    'nociception', 'pain', 'c. elegans', 'exosome', 'axotomy', 'locomotion',
    'synaptojanin', 'neuromuscular', 'global health', 'health outcome'];
  ...
  const econWords = ['economics', 'econom', 'business', 'market', 'policy', 'financial',
    ...
    'environmental', 'sustainability', 'climate', 'conservation', 'democrat',
    'mobilization', 'exam', 'student performance'];

  let techScore = 0, healthScore = 0, econScore = 0;
  for (const word of techWords) { if (text.includes(word)) techScore++; }
  for (const word of healthWords) { if (text.includes(word)) healthScore++; }
  for (const word of econWords) { if (text.includes(word)) econScore++; }
...
for (const entry of entries) {
  ...
  const role = determineRole(entry);
...
const seen = new Map();
const unique = [];
for (const entry of institutionStudents) {
...
console.log(`Total entries (${SOURCE_LABEL}): ${institutionStudents.length} → ${unique.length} after dedup`);
console.log(`Entries skipped (no valid email): ${skippedNoEmail}`);
...
// Count by role
const roleCounts = { team_lead: 0, mentor: 0 };
for (const e of unique) {
